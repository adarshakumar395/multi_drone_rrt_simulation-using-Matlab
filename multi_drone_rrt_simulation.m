function multi_drone_rrt_simulation
    ...
    interpolated_paths = cellfun(@(path) interpolate_path(path, 0.1), paths, 'UniformOutput', false);

    % Plot the paths
    colors = {'k--', 'b--', 'm--'};
    for k = 1:3
        if ~isempty(interpolated_paths{k})
            plot3(interpolated_paths{k}(:,1), interpolated_paths{k}(:,2), interpolated_paths{k}(:,3), colors{k}, 'LineWidth', 1);
        end
    end
    ...
    for i = 1:max_path_length
        ...
        for k = 1:3
            if i <= size(interpolated_paths{k}, 1)
                set(drone_plots(k), 'XData', drones(:, 1) + interpolated_paths{k}(i, 1), ...
...
function interpolated_path = interpolate_path(path, interval)
    ...
    interpolated_path = interp1(1:size(path, 1), path, linspace(1, size(path, 1), num_points));
end
